// /api/track com auto-descoberta de endpoint/parâmetros
export default async function handler(req, res) {
  ...
  // se o usuário definiu explicitamente o caminho e params, usa direto
  if (process.env.BRUDAM_ENDPOINT_PATH) {
    const pCnpj = process.env.BRUDAM_PARAM_CNPJ || "cnpj_remetente";
    const pDoc  = process.env.BRUDAM_PARAM_DOC  || "numero_nf";
  ...
  // Auto-descoberta: tenta combinações comuns
  const prefixes = ["", "/api"];
  ...
  const docKeys  = ["numero_nf", "nf", "documento", "numeroDocumento", "numero_doc", "numeroNfe", "nfe", "chave", "chave_nfe", "chaveNfe"];
  ...
  for (const pre of prefixes) for (const resPath of resources) for (const kC of cnpjKeys) for (const kD of docKeys) {
    ...
    try {
      ...
      if (r.status !== 404) {
        // Achamos um endpoint válido (200/400/401/403/500 já mostram que a rota existe)
        res.setHeader("X-Upstream-URL", url.toString());
